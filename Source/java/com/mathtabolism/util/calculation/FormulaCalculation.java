...
public class FormulaCalculation {
  ...
  public static Calorie calculateTDEE(double weight, double height, int age, Gender gender, UnitSystem unitSystem,
      TDEEFormula formula) {
    Measurement weightM = new Measurement(unitSystem.isMetric() ? UnitMeasurement.KILOGRAM : UnitMeasurement.POUND,
        weight);
    Measurement heightM = new Measurement(unitSystem.isMetric() ? UnitMeasurement.CENTIMETER : UnitMeasurement.INCH,
        height);
